...
if not logger.handlers:
    ...
    handler.setFormatter(
        logging.Formatter("[DLLM_DEBUG %(asctime)s] %(message)s", datefmt="%H:%M:%S")
    )
...
class LowConfidenceFDFO(DllmAlgorithm):
    ...
    def __init__(self, config: DllmConfig) -> None:
        ...
        self._call_count = 0
        logger.info(
            f"[INIT] LowConfidenceFDFO initialized with threshold={self.threshold}, "
    ...
    def run(
        ...
        logger.debug(
            f"[RUN #{call_id}] batch_size={batch_size}, mask_counts={mask_counts_cpu}"
        )
        ...
        for batch_id in range(batch_size):
            ...
            if mask_count == 0:
                ...
                logger.debug(
                    f"[RUN #{call_id}] batch={batch_id}: NO MASKS, block will be accepted. "
            ...
            logger.debug(
                f"[RUN #{call_id}] batch={batch_id}: {mask_count} masks at positions {mask_positions[:5]}..., "
            ...
            if num_above_threshold == 0:
                ...
                logger.debug(
                    f"[RUN #{call_id}] batch={batch_id}: FALLBACK - selecting pos {selected_pos} "
            ...
            logger.debug(
                f"[RUN #{call_id}] batch={batch_id}: After transfer, {remaining_masks} masks remaining"
            )
        ...
        for i in range(batch_size):
            ...
            if accept_len > 0:
                logger.info(
                    f"[RUN #{call_id}] batch={i}: ACCEPT block (no masks at start), "
                    f"tokens={next_token_ids[i][:5]}..."
                )
            else:
                logger.debug(
                    f"[RUN #{call_id}] batch={i}: INCOMPLETE (had {mask_counts_cpu[i]} masks at start)"
                )
